...
def possibleResults(numbers: List[Long]): Set[Long] = numbers match
  case x :: Nil => Set(x)
  case x :: xs => possibleResults(xs).map(x + _) ++ possibleResults(xs).map(x * _)
...
def possibleResults2(numbers: List[Long]): Set[Long] = 
  val operators = Seq((x: Long, y: Long) => x+y, (x: Long,y: Long) => x*y, concat)
  numbers match
    case x :: Nil => Set(x)
    case x :: xs => operators.flatMap(op => possibleResults2(xs).map(y => op(x,y))).toSet
